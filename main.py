...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost:3306/moviebuzz'
...
def fun():
    ...
    query1 = db.session.query(Movies,Rating).add_columns(Movies.poster,Movies.description,Movies.title,Movies.movie_id).order_by(Rating.rating).all()
    return render_template('index.html',query=query,query1=query1)

@app.route('/home')
def home():
    query = Movies.query.order_by(Movies.movie_id.desc()).all()
    query1 = db.session.query(Movies,Rating).add_columns(Movies.poster,Movies.description,Movies.title,Movies.movie_id).order_by(Rating.rating.desc()).all()
    return render_template('index.html',query=query,query1=query1)
